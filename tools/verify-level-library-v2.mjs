...
// v0.15.72: expansion-v3-* packs (Fast Generator v3) come on top of the frozen 762-level v2 library.
const ids=new Set(),structures=new Set(),counts={},expansion={};let total=0,v3=0;
for(const packInfo of catalog.packs){
 ...
 for(const l of pack.levels){
  ...
  if(file.includes('expansion-v2-'))(expansion[size]??=Array(10).fill(0))[grade-1]++;
  if(file.includes('expansion-v3-'))v3++;
  if(process.argv.includes('--deep')&&/expansion-v[23]-/.test(file)){const measurement=classify({id:l.levelId,state});if(measurement.status!=='ok'||measurement.difficulty!==grade||measurement.raw!==l.difficulty.score)throw Error('Classifier mismatch '+l.levelId)}
  total++;
 }
}
if(total-v3!==762||Object.keys(counts).length!==6||Object.values(counts).some(a=>a.length!==10||a.some(n=>n<10))||Object.values(expansion).some(a=>a.some(n=>n!==5)))throw Error('Coverage failed: '+JSON.stringify({total,counts,expansion}));
console.log(JSON.stringify({total,v2Library:total-v3,expansionV3:v3,coverage:counts,newLevels:expansion,deep:process.argv.includes('--deep')},null,2));
