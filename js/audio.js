...
const AudioManager=(()=>{
 ...
 function noise(dur=.12,gain=.025,when=0,freq=650){if(!effectsEnabled)return;const c=context();if(!c||c.state!=='running')return;const len=Math.ceil(c.sampleRate*dur),buf=c.createBuffer(1,len,c.sampleRate),d=buf.getChannelData(0);for(let i=0;i<len;i++)d[i]=(Math.random()*2-1)*(1-i/len);const s=c.createBufferSource(),f=c.createBiquadFilter(),g=c.createGain();f.type='bandpass';f.frequency.value=freq;f.Q.value=.7;g.gain.value=gain;s.buffer=buf;s.connect(f).connect(g).connect(c.destination);s.start(c.currentTime+when)}
 return{
  ...
  bomb(){themed('bomb',()=>{noise(.55,.09,0,180);noise(.3,.06,.02,900);tone(95,.55,'sine',.09,0,34);tone(190,.2,'triangle',.035,0,70)})},
